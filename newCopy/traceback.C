...
vector<string> loadfilelist(void)
{
  ...
  //To generate an updated version of this list,
  //go to the directory to be processed and enter
  //find . -name "ebuilt*" > valideventfilelist.txt
...
//input text file
 ...
 validfilearray.pop_back();//For some reason the last one is double counted
 //I may or may not investigate further; the answer will probably just make me mad
...
{//Does the actual work of the program. The main function is just reserved for fancy I/O
  ...
  //Load in the files to be processed
  vector<string> validfilearray;
   validfilearray = loadfilelist();
   TChain *chain = new TChain("sumtree");
   cout << validfilearray.size() << endl;
  ...
  chain->SetBranchAddress("hitenergy",&hitenergy); //hit energy (hit level information)
  chain->SetBranchAddress("MJDeventenergy",&MJDeventenergy); //eventt energy (event level information)
  chain->SetBranchAddress("detectornumber",&detectornumber);   //hit level information
  chain->SetBranchAddress("R90",&R90);
  //G4event branch has been deprecated; we only need a filenumber and a hitnumber for tracing back
  ...
  while(keepgoing)
	...
	case 3: //hit energy
	  ...
	  switch(comparatorchoice)
	      ...
	      for(i=0;i<entries;i++)
		{
		  ...
		  for(j=0;j<hitenergy->size();j++)
		    {
		      if(hitenergy->at(j)>=numbertocompare)
			{
			  subpassed[(subentryplaceholder[i]+j)]=0;
			}		      
		    }		  
	...
	case 4: //event energy
	  ...
	  switch(comparatorchoice)
	      ...
	      for(i=0;i<entries;i++)
		{
		  ...
		  if(MJDeventenergy>=numbertocompare)		  
		    {
		      passed[i] = 0;
		      for(j=0;j<hitenergy->size();j++)
			{
	  ...
	  break;//cutchoice 4 - event energy
	...
	case 5: //detector number
	  ...
	  switch(comparatorchoice)
	      ...
	      for(i=0;i<entries;i++)
		{
		  ...
		  for(j=0;j<detectornumber->size();j++)
		    {
		      if(detectornumber->at(j)>=numbertocompare)
			{
			  subpassed[(subentryplaceholder[i]+j)]=0;
			}		      
		    }		  
	...
	case 6: //R90
	  ...
	  switch(comparatorchoice)
	      ...
	      for(i=0;i<entries;i++)
		{
		  ...
		  //Comb through vectors for R90 values
		  for(j=0;j<R90->size();j++)
		    {
		      if(R90->at(j)>=numbertocompare)
			{
			  subpassed[(subentryplaceholder[i]+j)]=0;
			}		      
		    }		  
	...
	case 7: //multiplicity
	  ...
	  break;//cutchoice 7 - multiplicity
  ...
  for(i=0;i<entries;i++)
    {
      ...
      for(j = subentryplaceholder[i];j<(subentryplaceholder[i]+hitenergy->size());j++)
	{
	  ...
	  if(subpassed[j])
	    {//If any one of the hits is non-empty, the event is treated as non-empty as well
	      ...
	      output << filenumber << " " << hitnumber->at(j-subentryplaceholder[i]) << endl;
	      //output << G4event << " " << hitenergy->at(j-subentryplaceholder[i]) << " " << detector->at(j-subentryplaceholder[i]) << endl;
	    }//This is also where the writing to file is done (still being tested)
	}
      if(totallyempty && passed[i])
	{
	  cout << endl <<"Event " << i << " had all hits miss the cut, so it has been flagged as cut as well." << endl;
	  passed[i] = 0;
	}
    }
...
void traceback(void)
{
  ...
  cout << "Units of time are nanoseconds; units of energy are keV; units of length are mm" << endl << endl;
  cout << "Select which variable you would like to cut on." << endl << endl <<
    "1. start time" << endl <<
    "2. end time" << endl <<
    "3. hit energy sum" << endl <<
